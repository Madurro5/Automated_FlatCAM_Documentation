...
Ce guide d'utilisation vous acompagne pour convertir simplement vos fichiers `GERBER` en fichier `GCODE` en utilisant Automated FlatCAM. De l'ouverture du logiciel jusqu'à la récupération des GCODE en passant par la manipulation du plugin `Automated`, ce guide vous accompagne à toutes les étapes.
...
Il est également possible de l'ouvrir en allant dans le menu : "Plugins>Conversion Automatisé"
...
Si les étapes précédentes ont été ben respectées il vos sufit de cliquer sur `Lancer Conversion Automatique`.

Une fenêtre `Editeur de Script` ainsi que le terminal s'ouvrent ce qui est parfaitement normal. Après quelque instant si plus aucune commandes ou informations ne sont affichée sur le terminal alors vous pouvez cliquer sur la fenêtre `Zone de Dessin` à nouveau pour vérifier visuellement la bonne création des GCode. Vous pouvez retrouver ces dernier au répertoire que vous avez sélectionné.
...
Les géométries et les objets g-code on été générés. La face inférieure a été mise en miroir selon Y pour permettre l'usinage. Les objets g-code surlignés en jaunes ont déjà été enregistrés à l'emplacement indiqué dans la section `Répertoire de sortie`.

## Vous souhaitez usiner un dessin/logo/géométrie

Pour usiner une forme, un dessin ou des géométrie sur certains matériaux veuillez suivre les étapes suivantes.
...
### Sélectioner Workflow SVG (Geometry)
...
Par défaut, une configuration pour PCB est sélectionnée. S'assurer de bien avoir `Default Geometry` de sélectionné ou une autre configuration spéciale pour usiner des dessins ou formes 2D.

> **Note :** Il est important de noter que les fichiers SVG, étant importés directement comme géométrie, ne subissent pas d'opération d'isolement et que seule les paramètres de configuration lié à la tâche CNC (CNC Job) sont important (car seule opération de conversion). Les paramètres de perçage sont important également si un fichier de perçage (Excellon) est inséré.
...
Il ne reste plus qu'à importer les fichiers GCODE dans OpenBuild Control et préparer la CNC à un usinage.

[Lien vers le guide d'utilisation d'OpenBuild Control.](../openbuilds-control.md) 
